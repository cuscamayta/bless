...
function createInventoryOutput(request) {
    return {
        ...
        total: request.body.total,
        code: request.body.code,
        owner: request.body.deliveredby,
        typeprice: request.body.typeprice,
        type: 0,
        status: 1,
...
function createInventoryDetail(request, index, res) {
    var quantityDet = request.body.details[index].quantity;
    if (res.type == 0) {
        quantityDet = (request.body.details[index].quantity * - 1);
    }
    return {
